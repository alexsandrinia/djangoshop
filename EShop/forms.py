...
class LoginForm(forms.Form):
	...
	def clean(self):
		...
		if not User.objects.filter(username = username).exists():
			raise forms.ValidationError('Пользователь с даннным логином не зарегистрирован в системе! ')
...
class RegistrationForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['username'].label = 'Логин'
		self.fields['password'].label = 'Пароль'
		self.fields['password'].help_text = 'Придумайье пароль'
	...
	def clean(self):
		...
		if User.objects.filter(username = username).exists():
			raise forms.ValidationError('Пользователь с даннным логином уже зарегистрирован в системе! ')
		if User.objects.filter(email = email).exists():
			raise forms.ValidationError('Пользователь с такой почтой уже зарегистрирован в системе! ')
...
class OrderForm(forms.Form):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['phone'].help_text = 'Пожалуйста, указывайте реальный телефон, по которому с Вами можно связатсья'
		self.fields['buying_type'].label = 'Способ получения'
		self.fields['delivery_address'].label = 'Адрес доставки'
		self.fields['delivery_address'].help_text = 'Обязательно указываетй город!'
